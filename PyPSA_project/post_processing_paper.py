...
ds = dataset('output/prime_euro_80_storage_5D_eta_0.45.xlsx',
...
case_scenarios = [0,                                        # Optimum
                ...
                ds.df_sum_var.solar.idxmax(),                # low backup
...
fig = plot_network('data/networks/euro_80_storage',
                    ds,case_scenarios,['(a) Optimum','(b) High equality','(c) Large solar capacity','(d) Low co2 emission'])
fig.show()
...
def calc_interrior(ds):
    ...
    ds.df_interrior['transmission'] =  griddata(ds.df_sum_var[variables],
            ds.df_sum_var['transmission'],
            ds.interrior_points, 
            method='linear') 

    ds.df_interrior['ocgt'] =  griddata(ds.df_sum_var[variables],
            ds.df_sum_var['ocgt'],
            ds.interrior_points, 
            method='linear') 

    for key in ds.df_secondary_metrics.keys():
        ds.df_interrior[key] = griddata(ds.df_sum_var[variables],
...
def plot_contour(x,y,plot_range=None):
    ...
    h,x_bins,y_bins = np.histogram2d(x,y,bins=40,
                                range=plot_range,
                                )
    ...
    line_smoothing=1.3,
    showscale = False,
    contours=dict(start=0.5,end=400,size=150),
...
def figure3(ds,scenario_idx):
    ...
    label_dic = {'H2': 'Hydrogen storage',
                'wind': 'Wind turbines',
                'solar': 'Solar PV',
                'battery': 'Battery',
                'gini': 'Gini coefficient',
                'system_cost': '% Cost increase',
                'co2_emission': '% CO2 reduction',
                'ocgt': 'Backup capacity',
    ...
    for i in range(data_shape):
        for j in range(data_shape):
            ...
            if i != j and i>=j:
                # Plot scatter
                for idx,color,symbol in zip(scenario_idx[::-1],['red','gray','black','black'][::-1],['circle','asterisk','x-thin','hash'][::-1]):
            ...
            if i == data_shape-1 :
                ...
                fig.add_annotation(dict(
                                        ...
                                        x=[0,0.12 ,0.26 , 0.43, 0.57 , 0.70 ,0.87,0.96][j],
                                        showarrow=False,
                                        align="left",
                                        text=labels[j],
                                        textangle=0,
...
#%%
fig = figure3(ds,case_scenarios)

#fig.write_image('figure3.pdf')
...
fig.add_trace(go.Scatter(x=[0],y=[0],text=['Optimum'],mode='markers+text',textposition='middle right',
...
fig.add_trace(go.Scatter(x=[2],y=[0],text=['Low CO2 emission'],mode='markers+text',textposition='middle right',
                        marker=dict(color='orange',size=12,symbol='hash',
                                        line=dict(width=2,color='orange'))))

fig.add_trace(go.Scatter(x=[3],y=[0],text=['Large Solar PV capacity'],mode='markers+text',textposition='middle right',
...
fig.show()
fig.write_image('figure3_legend.pdf')
# %% CSV files
...
def calc_metrics(ds,max_co2):
    ...
    # Land use 
    ds.df_secondary_metrics['land_use'] = calc_land_use(ds)

    # Implementation time
    ds.df_secondary_metrics['implementation_time'] = calc_implementation_time(ds)

    ds.df_secondary_metrics['gini_capex'] = calc_gini_capex(ds)

    #

    return ds

def calc_gini_capex(ds):
...
def calc_implementation_time(ds):
    network = pypsaTools.import_network('data/networks/euro_30')
    buses = network.buses.index
    techs = ['onwind','offwind','solar','ocgt']
...
def plot_gini(x_data,y_data,fig,row,names = ['1% slack', '5% slack', '10% slack',],x_title='Gini coefficient',y_title='CO2 emission reduction [%]'):
...
fig = make_subplots(rows = 5, cols=1,shared_xaxes=True,shared_yaxes=False,vertical_spacing=0.05
                    #subplot_titles=['Gini','Land use','Gini capex','Implementation time']
                    )
...
fig.update_yaxes(title_text='Gini - Energy production ',range=[0,0.7],showgrid=False,linecolor='black',row=1,col=1)
fig.update_yaxes(title_text='Gini - Investement',range=[0,0.7],showgrid=False,row=2,linecolor='black',col=1)
fig.update_yaxes(title_text='Land use [km2]',showgrid=False,linecolor='black',row=3,col=1)
fig.update_yaxes(title_text='Implementation time',range=[0,30],showgrid=False,linecolor='black',row=4,col=1)
fig.update_yaxes(title_text='Transmission capacity [GW]',range=[0,2000],showgrid=False,linecolor='black',row=5,col=1)
...
fig.update_xaxes(title_text='CO2 reduction',range=[0,100],showgrid=False,linecolor='black',row=5,col=1)
