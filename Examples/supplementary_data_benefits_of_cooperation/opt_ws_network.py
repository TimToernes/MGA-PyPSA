...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        print("No options file specified, using default options.yml")
    ...
    options = yaml.load(open(options_file_name,"r"),Loader=yaml.FullLoader)
    
    network = init_model(options)
    network = solve_model(network)
     
    network.export_to_hdf5('./network_csv/euro-30')
